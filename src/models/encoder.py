...
def build_encoder_only_model(
    ...
    x = keras_nlp.layers.TransformerEncoder(
        intermediate_dim=256, num_heads=4, name="transformer_encoder_block_1", dropout=0.3
    ...
    x = keras_nlp.layers.TransformerEncoder(
        ...
        num_heads=4,
        dropout=0.2,
        name="transformer_encoder_block_2",
    )(x, padding_mask=padding_mask)

    outputs = layers.Dense(num_tags, activation="softmax", name="tag_classifier")(x)

    model = keras.Model(inputs, outputs, name="Transformer_EncoderOnly")
    return model
...
if __name__ == "__main__":
    ...
    embedding_dim = 300
    glove_path = f"/content/drive/MyDrive/NLP/data/glove.6B/glove.6B.{embedding_dim}d.txt"
    ...
    print("6. Gerando Avaliação e Matriz de Confusão...")
    ...
    generate_confusion_matrix(
        ...
        filepath="encoder_confusion_matrix.png",
    )

    print("7. Avaliação Final do Modelo...")
