...
\begin{itemize}
    ...
    \item \textbf{Hiperparâmetros Fixos:}  Para isolar a influência de cada arquitetura, todos os experimentos compartilharam o mesmo conjunto de hiperparâmetros. Utilizou-se uma camada de \textit{Embedding} inicializada com os pesos pré-treinados do GloVe de 50 dimensões e uma camada recorrente com 128 unidades ocultas. O treinamento foi conduzido com o otimizador Adam, função de perda \textit{Sparse Categorical Crossentropy} e batch size de 32. Embora o teto de treinamento tenha sido estipulado em 30 épocas, aplicou-se a técnica de \textit{Early Stopping} como critério de parada. Esse mecanismo foi configurado com paciência de três épocas: caso a perda de validação não apresentasse redução nesse intervalo, o processo era interrompido, restaurando-se os pesos do modelo que obteve a menor perda global.
\end{itemize}

\subsubsection{Resultados Gerais e Comparativos}
Os quatro modelos implementados (\textit{RNN} Unidirecional, \textit{LSTM} Unidirecional, \textit{RNN} Bidirecional e \textit{LSTM} Bidirecional) foram avaliados rigorosamente no conjunto de teste, composto por 129.654 tokens ativos (desconsiderando-se os preenchimentos de \textit{padding}). Para mensurar e contrastar o desempenho das arquiteturas, adotaram-se as seguintes métricas:

\begin{itemize}
\item \textbf{Acurácia Global:} Mede a proporção de predições corretas em relação ao total de tokens avaliados, fornecendo uma visão geral da eficácia do modelo;

\item \textbf{Média Macro da $F_1$ (\textit{Macro Avg}):} Calculada a partir da média aritmética simples do $F_1$-Score de todas as classes gramaticais, onde o $F_1$ de cada classe consiste na média harmônica entre a sua precisão e a sua revocação (\textit{recall}). Por atribuir o mesmo peso a todas as etiquetas, independentemente de sua frequência de ocorrência, essa métrica é fundamental para avaliar a robustez do modelo diante do desbalanceamento do \textit{dataset}, penalizando a pontuação final caso ocorram erros frequentes em classes minoritárias.

\item \textbf{Tempo por Passo:} Indica o custo computacional temporal despendido por etapa de processamento durante o treinamento. Essa métrica é essencial para analisar o \textit{trade-off} de engenharia, determinando se o ganho em acurácia de uma arquitetura complexa justifica o seu custo de execução.

\end{itemize}

Os resultados globais consolidados após a conclusão dos experimentos, considerando os critérios supracitados, estão apresentados na Tabela 1.

\begin{longtable}{|C{4cm}|C{3cm}|C{4cm}|C{2cm}|}
\hline
\textbf{Arquitetura} & \textbf{Acurácia Global} & \textbf{Média Macro da F1}  & \textbf{Tempo / Passo} \\ \hline
RNN Unidirecional  & 90,88\% & 0,8269  & 22,75ms \\ \hline
LSTM Unidirecional & 92,10\% & 0,8465 & 9,86ms \\ \hline
RNN Bidirecional   & 94,16\% & 0,8761 & 41.6ms \\ \hline
LSTM Bidirecional  & 94,68\% & 0,8902 & 14.17ms \\ \hline
...
\begin{itemize}
\item \textbf{Análise do Desempenho Global e o Impacto da Bidirecionalidade:} Os resultados revelam que a inclusão do fluxo bidirecional de informações foi o fator determinante para o ganho expressivo de desempenho no experimento. A \textit{LSTM Bidirecional} consolidou-se no topo, atingindo a maior acurácia global (94,68\%) e a maior Média Macro da F1 (0,8902), seguida de perto pela \textit{RNN Bidirecional} (94,16\% de acurácia e 0,8761 de F1). Este comportamento evidencia que, para a tarefa de \textit{POS Tagging}, ter acesso simultâneo ao histórico e ao contexto futuro da sentença é um requisito crítico para elevar o teto de acertos gerais do modelo.

\item \textbf{O Diferencial das Células LSTM:} Além do ganho contextual trazido pela bidirecionalidade, os mecanismos de portas da LSTM mostraram-se superiores às células de RNN simples em cenários equivalentes. Na comparação direta entre os modelos unidirecionais, a \textit{LSTM Unidirecional} superou a \textit{RNN Unidirecional} tanto em acurácia (92,10\% contra 90,88\%) quanto em Média Macro da F1 (0,8465 contra 0,8269). O mesmo padrão de superioridade da LSTM se repete no cenário bidirecional, comprovando que a capacidade de reter informações de longo prazo mitiga erros em sentenças mais complexas.

\item \textbf{Sensibilidade das Arquiteturas Unidirecionais:} Os modelos baseados em fluxo unidirecional mostraram as maiores limitações do experimento. A \textit{RNN Unidirecional} obteve os menores índices gerais (90,88\% de acurácia e 0,8269 de F1 Macro). Esse comportamento corrobora a hipótese de que processar o texto em apenas um sentido limita a capacidade da rede de resolver ambiguidades gramaticais cuja resolução depende estritamente de palavras ou pistas sintáticas que aparecem adiante na estrutura da sentença.

\item \textbf{Análise de Eficiência Computacional e quebra de paradigma:} Sob a perspectiva de engenharia, a análise do tempo por passo (\textit{Tempo / Step}) trouxe o resultado mais surpreendente do experimento. Ao contrário do esperado teoricamente, as arquiteturas baseadas em LSTM foram significativamente mais velozes do que as RNNs simples durante o treinamento. A \textit{LSTM Unidirecional} foi o modelo mais rápido do experimento (9,86ms), operando em menos da metade do tempo da \textit{RNN Unidirecional} (22,75ms). Da mesma forma, a campeã de desempenho \textit{LSTM Bidirecional} exigiu apenas 14,17ms por passo, cerca de três vezes menos tempo que os 41,60ms demandados pela \textit{RNN Bidirecional}. Esse cenário configura um caso ideal de tomada de decisão: a \textit{LSTM Bidirecional} é a melhor escolha, entregando a máxima qualidade linguística com o menor custo de processamento temporal.
...
Ao contrastar analiticamente o comportamento das redes a partir dos relatórios de classificação, nota-se uma nítida mudança no padrão de dispersão dos erros. Nas redes unidirecionais (\textit{RNN} e \textit{LSTM} Unidirecionais), há uma queda acentuada de desempenho em classes que dependem fortemente de determinantes posteriores. O caso mais emblemático ocorre na etiqueta de adjetivos (\texttt{JJ}), cujo F1-score fica em $0,7438$ na RNN e $0,7585$ na LSTM, indicando que esses modelos favorecem classes majoritárias como substantivos (\texttt{NN}) quando o fluxo de informação é limitado a apenas um sentido.

Em contrapartida, as arquiteturas bidirecionais demonstram uma contração drástica desse ruído, confinando os erros estritamente aos pares linguísticos de altíssima ambiguidade sintática. Ao introduzir o contexto futuro, o F1-score para \texttt{JJ} salta expressivamente para $0,8539$ na RNN Bidirecional e atinge o ápice de $0,8667$ na LSTM Bidirecional.

No escopo dos métodos bidirecionais, a arquitetura de portas da LSTM demonstrou superioridade analítica ao mitigar erros de classificação tanto em etiquetas complexas de verbos quanto no tratamento de classes minoritárias (de baixo suporte). Como exemplo, para a tag de verbos no gerúndio (\texttt{VBG}), a LSTM Bidirecional conteve a dispersão de falsos positivos mantendo o equilíbrio das métricas, enquanto a RNN Bidirecional, apesar de um bom desempenho global, tendeu a sofrer mais oscilações nessas estruturas flexíveis. Ao filtrar de maneira controlada as informações provenientes de ambas as direções da sequência, a LSTM foi capaz de refinar as fronteiras de decisão, consolidando a maior Média Macro da F1 ($0,8902$) de todo o experimento.
